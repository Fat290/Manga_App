...
class _DragableWidgetState extends State<DragableWidget>
  ...
  double get currentAngle {
    final currentPositionX = currentPosition.dx; // Cache for readability
    final screenWidth = screenSize.width;
    final widgetWidth = size.width;

    if (currentPositionX < 0) {
      return (pi * 0.2) * currentPositionX ;
    } else if (currentPositionX + widgetWidth > screenWidth) {
      return (pi * 0.2) * (currentPositionX + widgetWidth - screenWidth) / widgetWidth;
    } else {
      return 0.0;
    }
  }
